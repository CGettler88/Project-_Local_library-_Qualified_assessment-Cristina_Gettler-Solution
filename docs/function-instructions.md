...
It returns a _number_ that represents (the number of times the account's) ID appearing in any book's `borrows` array.
add up all the number of times an ID appears in borrows object
